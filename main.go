...
func main() {
	...
	r.Handle("/{item:stock}/{id}", securechain.Then(env.AppMiddleware(env.GetEntityStockHandler))).Methods("GET")
	...
	r.Handle("/f/{item:stock}/{id}", securechain.Then(env.AppMiddleware(env.FakeHandler))).Methods("GET")
	// people
