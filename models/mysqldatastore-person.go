...
func (db *MySQLDataStore) ValidateCaptcha(token string, text string) (bool, error) {
...
func (db *MySQLDataStore) InsertCaptcha(data *captcha.Data) (string, error) {
...
func (db *MySQLDataStore) GetPeople(p helpers.DbselectparamPerson) ([]Person, int, error) {
	var (
		people                             []Person
		isadmin                            bool
		count                              int
...
func (db *MySQLDataStore) GetPerson(id int) (Person, error) {
	var (
		person Person
		sqlr   string
		err    error
...
func (db *MySQLDataStore) GetPersonByEmail(email string) (Person, error) {
	var (
		person Person
		sqlr   string
		err    error
...
func (db *MySQLDataStore) GetPersonPermissions(id int) ([]Permission, error) {
	var (
		ps   []Permission
		sqlr string
		err  error
...
func (db *MySQLDataStore) GetPersonManageEntities(id int) ([]Entity, error) {
	var (
		es   []Entity
		sqlr string
		err  error
...
func (db *MySQLDataStore) GetPersonEntities(LoggedPersonID int, id int) ([]Entity, error) {
	var (
		entities []Entity
		isadmin  bool
...
func (db *MySQLDataStore) DoesPersonBelongsTo(id int, entities []Entity) (bool, error) {
	var (
		sqlr  string
		count int
		err   error
...
func (db *MySQLDataStore) HasPersonPermission(id int, perm string, item string, itemid int) (bool, error) {
...
func (db *MySQLDataStore) DeletePerson(id int) error {
	var (
		sqlr string
		err  error
	)
...
func (db *MySQLDataStore) CreatePerson(p Person) (error, int) {
	var (
		sqlr   string
		res    sql.Result
		lastid int64
...
func (db *MySQLDataStore) UpdatePersonPassword(p Person) error {
	var (
		sqlr  string
		err   error
		hpass []byte
...
func (db *MySQLDataStore) UpdatePerson(p Person) error {
	var (
		sqlr string
		err  error
	)
...
func (db *MySQLDataStore) IsPersonAdmin(id int) (bool, error) {
	var (
		res   bool
		count int
		sqlr  string
...
func (db *MySQLDataStore) IsPersonManager(id int) (bool, error) {
	var (
		res   bool
		count int
		sqlr  string
...
func (db *MySQLDataStore) insertPermissions(p Person) error {
	var (
		sqlr string
		err  error
	)
