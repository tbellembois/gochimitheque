...
window.operateEvents = {
    ...
    //             method: "DELETE",
    //         }).done(function(data, textStatus, jqXHR) {
    //             gjsUtils.message("store location deleted", "success");
    //             var $table = $('#table');
    ...
    'click .delete': function (e, value, row, index) {
        ...
        // ask for confirmation and then delete
        $("button#delete" + row.storelocation_id.Int64).on("click", function () {
            $.ajax({
                url: proxyPath + "storelocations/" + row['storelocation_id']['Int64'],
                method: "DELETE",
            }).done(function (data, textStatus, jqXHR) {
                gjsUtils.message(gjsUtils.translate("storelocation_deleted_message", container.PersonLanguage), "success");
                var $table = $('#table');
                $table.bootstrapTable('refresh');
            }).fail(function (jqXHR, textStatus, errorThrown) {
                handleHTTPError(jqXHR.statusText, jqXHR.status)
...
var createCallBack = function createCallback(data, textStatus, jqXHR) {
    gjsUtils.message(gjsUtils.translate("storelocation_created_message", container.PersonLanguage) + ": " + data.storelocation_name.String, "success");
...
var updateCallBack = function updateCallback(data, textStatus, jqXHR) {
    ...
    $table.bootstrapTable('refresh');
    gjsUtils.message(gjsUtils.translate("storelocation_updated_message", container.PersonLanguage) + ": " + data.storelocation_name.String, "success");
}
