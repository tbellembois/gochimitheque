...
function handleHTTPError(msgText, msgStatus) {
    ...
    switch (msgStatus) {
        case 401:
            gjsUtils.message(msgText, "danger");
            break;
        case 403:
            gjsUtils.message(msgText, "danger");
            break;
        case 500:
            ...
            break;
        default:
            gjsUtils.message(msgText, "light");
            break;
    }
}
...
function scanQR() {
    ...
    QrScanner.hasCamera().then(hasCamera => {
        if (hasCamera) {
            $("div#video").append("<button type='button' onclick='closeQR()' class='btn btn-link'><span class='mdi mdi-close-box mdi-24px iconlabel'>" + gjsUtils.translate("close", container.PersonLanguage) + "</span></button>");
            window.qrScanner.start();
        } else {
            alert(gjsUtils.translate("nocamera", container.PersonLanguage));
            closeQR();
        }
    });
