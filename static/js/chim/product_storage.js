...
$('#table').on('load-success.bs.table refresh.bs.table', function () {
    ...
    if (urlParams.has("product")) {
        ...
        hasPermission("storages", "-2", "PUT").done(function () {
            ...
            i = $("<span>").addClass("mdi mdi-24px mdi-forklift iconlabel").html(gjsUtils.translate("storeagain_text", container.PersonLanguage));
            b.append(i);
            $("#button-store").html(b);
        })
        ...
        hasPermission("storages", "-2", "GET").done(function () {
            ...
            i = $("<span>").addClass("mdi mdi-24px mdi-sigma iconlabel").html(gjsUtils.translate("totalstock_text", container.PersonLanguage));;
            b.append(i);
            $("#button-stock").html(b);
    ...
    $.when(entitypromise, storelocationpromise, storagepromise, productpromise, bookmarkpromise).done(function () {
        ...
        if (bookmarkData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(gjsUtils.translate("menu_bookmark", container.PersonLanguage), "bookmark"));
        };
        if (historyData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(gjsUtils.translate("storage_history", container.PersonLanguage), "history"));
        };
        if (entityData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(entityData.entity_name, "entity"));
        };
        if (storelocationData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(storelocationData.storelocation_name.String, "storelocation"));
        };
        if (storageData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(storageData.product.name.name_label + " " + storageData.product.product_specificity.String + " - " + storageData.storelocation.storelocation_fullpath, "storage"));
        };
        if (productData !== undefined) {
            $("#filter-item").append(gjsWidgets.title(productData.name.name_label + " " + productData.product_specificity.String, "product"));
        };
    });
...
function queryParams(params) {
    ...
    var borrowing = urlParams.get("borrowing");
    var storage_to_destroy = urlParams.get("storage_to_destroy");
    ...
    if (borrowing == "true") {
        $("#advancedsearch").collapse('show');
        $('#s_borrowing').prop('checked', true);
        $('#s_borrowing').after("<span style='position: absolute; left: -5px; color: orange;' class='mdi mdi-checkbox-blank blink'></span>");
    }
    if (storage_to_destroy == "true") {
        $("#advancedsearch").collapse('show');
        $('#s_storage_to_destroy').prop('checked', true);
        $('#s_storage_to_destroy').after("<span style='position: absolute; left: -5px; color: orange;' class='mdi mdi-checkbox-blank blink'></span>");
    }
    ...
    if ($('#s_borrowing:checked').length > 0) {
        params["borrowing"] = true;
    }
    if ($('#s_storage_to_destroy:checked').length > 0) {
        params["storage_to_destroy"] = true;
    }
