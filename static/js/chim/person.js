$(document).ready(function () {
    ...
    $.fn.select2.amd.require([
        ...
        SelectAll.prototype.render = function (decorated) {
            ...
            var $selectAll = $('<a/>').addClass('btn btn-info').text(gjsUtils.translate("select_all", container.PersonLanguage));
            var $selectNone = $('<a/>').addClass('btn btn-info').text(gjsUtils.translate("none", container.PersonLanguage));
    ...
    $('select#entities').on('select2:unselecting', function (e) {
        ...
        if (ismanager) {
            gjsUtils.message(gjsUtils.translate("person_can_not_remove_entity_manager", container.PersonLanguage), "success");
            e.preventDefault();
        } else {
    ...
    $('select#entities').on('select2:select', function (e) {
        ...
        $("#permissions").append(gjsWidgets.permission(data.entity_id, data.entity_name));
    });
...
$('#table').on('load-success.bs.table refresh.bs.table', function () {
    $("table#table").find("tr[person_id]").each(function (index, b) {
...
window.operateEvents = {
    ...
    'click .delete': function (e, value, row, index) {
        ...
        $("button#delete" + row.person_id).on("click", function () {
            $.ajax({
                url: proxyPath + "people/" + row['person_id'],
                method: "DELETE",
            }).done(function (data, textStatus, jqXHR) {
                gjsUtils.message(gjsUtils.translate("person_deleted_message", container.PersonLanguage), "success");
                var $table = $('#table');
    ...
    $.when(personpromise, managerpromise, entitypromise, permissionpromise).done(function () {
        ...
        for (var i in entitydata) {
            if ($.inArray(entitydata[i].entity_id, managedentitydataids) == -1) {
                $("#permissions").append(gjsWidgets.permission(entitydata[i].entity_id, entitydata[i].entity_name, false));
            } else {
                $("#permissions").append(gjsWidgets.permission(entitydata[i].entity_id, entitydata[i].entity_name, true));
            }
        }
        ...
        if ($("input.perm").length > 0) {
            ...
            gjsWidgets.permissionPopulate(permissiondata);
        }
...
var createCallBack = function createCallback(data, textStatus, jqXHR) {
    gjsUtils.message(gjsUtils.translate("person_created_message", container.PersonLanguage) + ": " + data.person_email, "success");
...
var updateCallBack = function updateCallback(data, textStatus, jqXHR) {
    ...
    $table.bootstrapTable('refresh');
    gjsUtils.message(gjsUtils.translate("person_updated_message", container.PersonLanguage) + ": " + data.person_email, "success");
}
function savePersonp() {
    ...
    $.ajax({
        ...
        gjsUtils.message(gjsUtils.translate("person_password_updated_message", container.PersonLanguage), "success");
