...
$(document).ready(function () {
        ...
        rules: {
            ...
            unit_concentration: {
                required: function(element) {
                    return $("#storage_concentration").val() != "";
                  }            
            },
        },
        messages: {
    ...
    //
    // unit_concentration select2
    // 
    $('select#unit_concentration').select2({
        ...
        ajax: {
            ...
            processResults: function (data, params) {
                ...
                // getting the number of loaded select elements
                selectnbitems = $("ul#select2-unit_concentration-results li").length + 10;
    ...
    $('select#unit_quantity').select2({
        ...
        ajax: {
            ...
            processResults: function (data, params) {
                ...
                // getting the number of loaded select elements
                selectnbitems = $("ul#select2-unit_quantity-results li").length + 10;
...
$('#s_storage_archive_button').on('click', function () {
    ...
    if ($('#s_storage_archive_button').hasClass("active")) {
        ...
        btntitle = gjsUtils.translate("showdeleted_text", container.PersonLanguage);
        btnicon = "delete";
    } else {
        updateQueryStringParam("storage_archive", "true");
        btntitle = gjsUtils.translate("hidedeleted_text", container.PersonLanguage);
    ...
    if (hasStock) {
        ...
        for (var i in sl.stock) {
            ...
            if (stock.total === 0 && stock.current === 0) {
                ...
                html.push("<div class='col'><span class='iconlabel'>" + gjsUtils.translate("stock_storelocation_title", container.PersonLanguage) + "</span> " + stock.current + " <b>" + stock.unit.unit_label.String + "</b></div>");
                html.push("<div class='col'><span class='iconlabel'>" + gjsUtils.translate("stock_storelocation_sub_title", container.PersonLanguage) + "</span> " + stock.total + " <b>" + stock.unit.unit_label.String + "</b></div>");
...
function showStock(pid) {
    $.ajax({
        url: proxyPath + "entities/stocks/" + pid,
        method: "GET",
    }).done(function (data, textStatus, jqXHR) {
        $("#stock-body").html("");
        for (var key in data) {
...
function detailFormatter(index, row) {
    ...
    html.push("<div class='row mb-sm-3'>")
    html.push("<div class='col-sm-6'><span class='iconlabel'>" + gjsUtils.translate("storage_quantity_title", container.PersonLanguage) + "</span> " + row["storage_quantity"]["Float64"] + " " + row["unit_quantity"]["unit_label"]["String"] + "</div>")
    html.push("<div class='col-sm-6'><span class='iconlabel'>" + gjsUtils.translate("storage_barecode_title", container.PersonLanguage) + "</span> " + row["storage_barecode"]["String"] + "</div>")
    html.push("</div>")

    html.push("<div class='row mb-sm-3'>")
        html.push("<div class='col-sm-6'>")
            if (row["storage_concentration"]["Valid"]) {
                html.push("<span class='iconlabel'>" + gjsUtils.translate("storage_concentration_title", container.PersonLanguage) + "</span> " + row["storage_concentration"]["Int64"])
            }
    ...
    if (row["storage_batchnumber"]["Valid"] && row["storage_batchnumber"]["String"] != "") {
        html.push("<div class='col-sm-6'><span class='iconlabel'> " + gjsUtils.translate("storage_batchnumber_title", container.PersonLanguage) + "</span> " + row["storage_batchnumber"]["String"] + "</div>")
    }
    if (row["supplier"]["supplier_label"]["Valid"]) {
        html.push("<div class='col-sm-6'><span class='iconlabel'> " + gjsUtils.translate("supplier_label_title", container.PersonLanguage) + "</span> " + row["supplier"]["supplier_label"]["String"] + "</div>")
    }
    html.push("</div>")

    html.push("<div class='row mb-sm-3'>")
    if (row["storage_entrydate"]["Valid"]) {
        html.push("<div class='col-sm-12'><span class='iconlabel'> " + gjsUtils.translate("storage_entrydate_title", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_entrydate"]["Time"], null, null, null) + "</div>")
    }
    if (row["storage_exitdate"]["Valid"]) {
        html.push("<div class='col-sm-12'><span class='iconlabel'> " + gjsUtils.translate("storage_exitdate_title", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_exitdate"]["Time"], null, null, null) + "</div>")
    }
    if (row["storage_openingdate"]["Valid"]) {
        html.push("<div class='col-sm-12'><span class='iconlabel'> " + gjsUtils.translate("storage_openingdate_title", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_openingdate"]["Time"], null, null, null) + "</div>")
    }
    if (row["storage_expirationdate"]["Valid"]) {
        html.push("<div class='col-sm-12'><span class='iconlabel'> " + gjsUtils.translate("storage_expirationdate_title", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_expirationdate"]["Time"], null, null, null) + "</div>")
    }
    html.push("</div>")

    html.push("<div class='row mb-sm-3'>")
    if (row["storage_comment"]["Valid"] && row["storage_comment"]["String"] != "") {
        html.push("<div class='col-sm-12'><span class='iconlabel'> " + gjsUtils.translate("storage_comment_title", container.PersonLanguage) + "</span> " + row["storage_comment"]["String"] + "</div>")
    }
    html.push("</div>")

    html.push("<div class='row mb-sm-3'>")
    html.push("<div class='col-sm-8'><span class='iconlabel'> " + gjsUtils.translate("created", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_creationdate"], null, null, null) + " <span class='iconlabel'> " + gjsUtils.translate("modified", container.PersonLanguage) + "</span> " + dateFormatter(row["storage_modificationdate"], null, null, null) + "</div>")
...
function storage_quantityFormatter(value, row, index, field) {
    ...
    if (row.unit_quantity.unit_label.Valid) {
        ret += " " + row.unit_quantity.unit_label.String
    }
    return ret;
}
//
...
function operateFormatter(value, row, index) {
    ...
    var borrowingicon = "hand-okay",
        borrowingtitle = gjsUtils.translate("storage_borrow", container.PersonLanguage);
    if (row.borrowing.borrowing_id.Valid) {
        borrowingicon = "hand-pointing-left";
        borrowingtitle = gjsUtils.translate("storage_unborrow", container.PersonLanguage);
    }

    if (row.storage_archive.Valid && row.storage_archive.Bool) {
        // this is an archive
        var actions = [
            '<button id="clone' + sid + '" sid="' + sid + '" class="clone btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("storage_clone", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-content-copy"></span>',
            '</button>',
            '<button id="restore' + sid + '" sid="' + sid + '" class="restore btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("storage_restore", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-undo"></span>',
            '</button>',
            '<button id="delete' + sid + '" sid="' + sid + '" class="delete btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("delete", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-delete"></span>',
            '</button>'];
    } else if (row.storage.storage_id.Valid) {
        // this is an history
        var actions = [
            '<button id="clone' + sid + '" class="clone btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("storage_clone", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-content-copy"></span>',
            '</button>'];
    } else {
        // buttons are hidden by default
        var actions = [
            '<button id="edit' + sid + '" sid="' + sid + '" class="edit btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("edit", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-border-color"></span>',
            '</button>',
            '<button id="clone' + sid + '" sid="' + sid + '" class="clone btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("storage_clone", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-content-copy"></span>',
            '</button>',
            '<button id="archive' + sid + '" sid="' + sid + '" class="archive btn btn-link btn-sm" style="display: none;" title="' + gjsUtils.translate("delete", container.PersonLanguage) + '" type="button">',
            '<span class="mdi mdi-24px mdi-delete"></span>',
            '</button>',
    ...
    if (row.storage_hc != 0) {
        actions.push('<button id="history' + sid + '" class="history btn btn-link btn-sm" title="' + gjsUtils.translate("storage_showhistory", container.PersonLanguage) + '" type="button">');
        actions.push('<span class="mdi mdi-24px mdi-history"></span>');
        actions.push('</button>');
    }
...
window.operateEvents = {
    ...
    //             method: "PUT",
    //         }).done(function(data, textStatus, jqXHR) {
    //             gjsUtils.message("storage restored", "success");
    //             var $table = $('#table');
    //             $table.bootstrapTable('refresh');
    //         }).fail(function(jqXHR, textStatus, errorThrown) {
    //             handleHTTPError(jqXHR.statusText, jqXHR.status)
    ...
    'click .restore': function (e, value, row, index) {
        ...
        // ask for confirmation and then restore
        $("button#restore" + row.storage_id.Int64).on("click", function () {
            $.ajax({
                url: proxyPath + "storages/" + row['storage_id'].Int64 + "/r",
                method: "PUT",
            }).done(function (data, textStatus, jqXHR) {
                gjsUtils.message(gjsUtils.translate("storage_restored_message", container.PersonLanguage), "success");
                var $table = $('#table');
                $table.bootstrapTable('refresh');
            }).fail(function (jqXHR, textStatus, errorThrown) {
                handleHTTPError(jqXHR.statusText, jqXHR.status)
            });
        }).css("color", "red").find("span.mdi-undo").prop("title", gjsUtils.translate("confirm", container.PersonLanguage));
    },
    ...
    //             method: "DELETE",
    //         }).done(function(data, textStatus, jqXHR) {
    //             gjsUtils.message("storage trashed", "success");
    //             var $table = $('#table');
    //             $table.bootstrapTable('refresh');
    //         }).fail(function(jqXHR, textStatus, errorThrown) {
    //             handleHTTPError(jqXHR.statusText, jqXHR.status)
    ...
    'click .archive': function (e, value, row, index) {
        ...
        // ask for confirmation and then delete
        $("button#archive" + row.storage_id.Int64).on("click", function () {
            $.ajax({
                url: proxyPath + "storages/" + row['storage_id'].Int64 + "/a",
                method: "DELETE",
            }).done(function (data, textStatus, jqXHR) {
                gjsUtils.message(gjsUtils.translate("storage_trashed_message", container.PersonLanguage), "success");
                var $table = $('#table');
                $table.bootstrapTable('refresh');
            }).fail(function (jqXHR, textStatus, errorThrown) {
                handleHTTPError(jqXHR.statusText, jqXHR.status)
    ...
    //             method: "DELETE",
    //         }).done(function(data, textStatus, jqXHR) {
    //             gjsUtils.message("storage deleted", "success");
    //             var $table = $('#table');
    //             $table.bootstrapTable('refresh');
    //         }).fail(function(jqXHR, textStatus, errorThrown) {
    //             handleHTTPError(jqXHR.statusText, jqXHR.status)
    ...
    'click .delete': function (e, value, row, index) {
        ...
        // ask for confirmation and then delete
        $("button#delete" + row.storage_id.Int64).on("click", function () {
            $.ajax({
                url: proxyPath + "storages/" + row['storage_id'].Int64,
                method: "DELETE",
            }).done(function (data, textStatus, jqXHR) {
                gjsUtils.message(gjsUtils.translate("storage_deleted_message", container.PersonLanguage), "success");
                var $table = $('#table');
                $table.bootstrapTable('refresh');
            }).fail(function (jqXHR, textStatus, errorThrown) {
                handleHTTPError(jqXHR.statusText, jqXHR.status)
...
function saveBorrowing() {
    ...
    $.ajax({
        ...
        $("#borrow").modal("hide");
        gjsUtils.message(gjsUtils.translate("storage_borrow_updated", container.PersonLanguage), "success");
        var $table = $('#table');
        $table.bootstrapTable('refresh');
    }).fail(function (jqXHR, textStatus, errorThrown) {
        handleHTTPError(jqXHR.statusText, jqXHR.status);
...
function operateEdit(e, value, row, index) {
    ...
    $('input#storage_concentration').val(null);

    $('select#storelocation').val(null).trigger('change');
    $('select#storelocation').find('option').remove();
    $('select#unit_quantity').val(null).trigger('change');
    $('select#unit_quantity').find('option').remove();
...
var createCallBack = function createCallback(data, textStatus, jqXHR) {
    gjsUtils.message(gjsUtils.translate("storage_created_message", container.PersonLanguage), "success");
...
var updateCallBack = function updateCallback(data, textStatus, jqXHR) {
    ...
    //- $table.bootstrapTable('refresh');
    gjsUtils.message(gjsUtils.translate("storage_updated_message", container.PersonLanguage) + ": " + data.storage_id.Int64 + " updated", "success");
    setTimeout(function () { window.location = proxyPath + "v/storages?product=" + data.product.product_id + "&hl=" + data.product.product_id; }, 1000);
}
...
function saveStorage() {
    ...
    var product_id = $("input#product_id").val(),
        storage_concentration = $("input#storage_concentration").val(),
        ...
        unit_quantity = $('select#unit_quantity').select2('data')[0],
    ...
    if (unit_quantity !== undefined) {
        $.extend(data, {
            "unit_quantity.unit_id": unit_quantity.id,
            "unit_quantity.unit_label": unit_quantity.text,
        });
    }
    ...
    if (storage_concentration !== "") {
        $.extend(data, {
            "storage_concentration": storage_concentration,
        });
    }
    if (storage_entrydate !== "" && storage_entrydate !== "0001-01-01") {
        $.extend(data, {
            "storage_entrydate": storage_entrydate,
        });
    }
    if (storage_exitdate !== "" && storage_exitdate !== "0001-01-01") {
        $.extend(data, {
            "storage_exitdate": storage_exitdate,
        });
    }
    if (storage_openingdate !== "" && storage_openingdate !== "0001-01-01") {
        $.extend(data, {
            "storage_openingdate": storage_openingdate,
        });
    }
    if (storage_expirationdate !== "" && storage_expirationdate !== "0001-01-01") {
        $.extend(data, {
