...
// Request.
type DBGetStoragesReq struct {
	DBGetStorages []interface{} `json:"DBGetStorages"`
}

// string, int

// Response.
type DBGetStoragesOk struct {
	Ok json.RawMessage
}
type DBGetStoragesErr struct {
	Err string
}
...
func DBGetStorages(requestRawString string, person_id int) (json.RawMessage, error) {
	...
	if message, err = json.Marshal(DBGetStoragesReq{
		DBGetStorages: req,
	}); err != nil {
		return json.RawMessage{}, err
	}
